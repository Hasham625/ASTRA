...
from models.vit_multi_ln import vit_small
from models.vit_multi_ln import ViT  # single-LN usage: pass bn_names=None
import time
...
def main():
    ...
    bn_names = ['normal', 'pgd']
    model = vit_small(bn_names=bn_names, num_classes=num_classes)
    model.cuda()
    ...
    if args.checkpoint != '':
        ...
        # zero init fc
        state_dict['fc.weight'] = torch.zeros(num_classes, 384).cuda()
    ...
    model_save = vit_small(num_classes=num_classes) # ViT without dual LayerNorm
...
def cvt_state_dict(state_dict, args):
    # deal with dual LayerNorm (ViT) — extract one LN path from ln_list
        ...
        for name, item in state_dict.items():
            if 'ln_list' in name:
                state_dict_new[name.replace(
                    '.ln_list.{}'.format(args.bnNameCnt), '')] = item
