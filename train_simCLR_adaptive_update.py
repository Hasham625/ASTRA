...
from models.vit_multi_ln import vit_small, proj_head_vit
from utils import *
import torchvision.transforms as transforms
from torch.nn import DataParallel
from StrategyNet_ViT import ViT_Strategy_Small
...
parser.add_argument('--patch_size', type=int, default=4, help='ViT patch size')
...
def main():
    ...
    # auto-set img_size/patch_size for imagenet100
    if args.dataset == 'imagenet100':
        args.img_size = 224
        args.patch_size = 16

    # define model
    model = vit_small(img_size=args.img_size, patch_size=args.patch_size, bn_names=bn_names)

    ### Strategy model
    strategy_model = ViT_Strategy_Small(args, img_size=args.img_size, patch_size=args.patch_size).to(torch.device(f"cuda:{args.gpu_ids[0]}"))
    strategy_model = nn.DataParallel(strategy_model.to(device),device_ids=args.gpu_ids)

    ch = model.fc.in_features
    args.feat_dim = ch  # store for reshape operations (384 for ViT-Small)
    model.fc = proj_head_vit(ch, bn_names=bn_names, twoLayerProj=args.twoLayerProj)
...
def compute_reward(target_model, clean_inputs, adv_inputs,strategy_model,acl_ds):
    ...
    if args.r1 > 0.0:
        sim_loss = nt_xent(torch.stack((clean_features, adv_features), dim=1).reshape(-1, args.feat_dim))
...
def train(train_loader, model, strategy_model, optimizer, strategy_optimizer, scheduler, strategy_scheduler, epoch, log):
    ...
    for i, (inputs) in enumerate(train_loader):
        ...
        similarity_loss = nt_xent(torch.stack((features_clean, features_adv), dim=1).reshape(-1, args.feat_dim))
